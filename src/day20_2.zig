...
pub fn main() !void {
    ...
    // =============================================================================
    {
        ...
        for (modules, moudule_output_strings.items) |*module, output| {
            ...
            while (output_tokens.next()) |output_id_str| {
                ...
                if (output_module) |mod| {
                    try output_builder.append(mod);

                    if (mod.class == .conjunction) {
                        try mod.class.conjunction.inputs.append(.{
                            .id = module.id,
                            .signal = .low,
                        });
                    }
                    ...
                    try secondary_modules.append(.{
                        .id = output_id,
                        .class = .broadcaster,
                        .outputs = try allocator.alloc(*Module, 0),
                    });
                    try module_map.put(output_id, &secondary_modules.items[secondary_modules.items.len - 1]);
    ...
    // Press the button until exactly one low signal is sent to 'rx'.
    var pending_sends = RingQueue(PendingSend).init(allocator);
    defer pending_sends.deinit();

    var button_presses: usize = 1;
    while (true) : (button_presses += 1) {
        ...
        if (low_signals == 1) break;
    }

    const stdout = std.io.getStdOut().writer();
    try stdout.print("Button presses : {}\n", .{button_presses});
}
