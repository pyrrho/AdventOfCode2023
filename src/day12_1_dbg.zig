...
pub fn main() !void {
    ...
    const file = try std.fs.cwd().openFile("src/day12_sample_input.txt", .{});
    ...
    for (input.items) |e| {
        ...
        std.debug.print("'{s}' (", .{e.line});
        std.debug.print("{}", .{e.runs[0]});
        for (e.runs[1..]) |run| {
            std.debug.print(", {}", .{run});
        }
        std.debug.print("): {}\n", .{a});
...
var prefix_depth: u8 = 1;
fn debugPrefix() void {
    for (0..prefix_depth) |_| {
        std.debug.print("  ", .{});
    }
}
fn debugPrint(
...
fn getArrangements(
    ...
    debugPrefix();
    std.debug.print("Calculating arrangements for '{s}' (", .{input.line});
    std.debug.print("{}", .{input.current_run});
    for (input.runs) |r| {
        std.debug.print(", {}", .{r});
    }
    std.debug.print(")\n", .{});
    ...
    while (true) {
        ...
        if (i == input.line.len) {
            debugPrefix();
            std.debug.print("Reached end of line; this sequence is {s}\n", .{if (last_run_done) "valid" else "invalid"});
        ...
        switch (c) {
            '.' => {
                ...
                debugPrefix();
                std.debug.print("Found '.'s; moving forward {} spaces\n", .{run_len});
                debugPrint(input.line[i..], run, input.runs[r_i..]);
                continue;
            },
            '#' => {
                if (run == 0) {
                    if (r_i == input.runs.len) {
                        debugPrefix();
                        std.debug.print("Found '#'s while no runs are available; invalid sequence\n", .{});
                        return 0;
                    }
                    run = input.runs[r_i];
                    r_i += 1;
                    debugPrefix();
                    std.debug.print("Found '#'s while the current_run is 0. New current run is {}\n", .{run});
                }

                const run_len = (std.mem.indexOfAnyPos(u8, input.line, i, ".?") orelse input.line.len) - i;
                if (run_len > run) {
                    debugPrefix();
                    std.debug.print("Found {} '#'s; this exceeds the current run ({}); invalid sequence\n", .{ run_len, run });
                    return 0;
                }
                ...
                if (run == 0 and i < input.line.len) {
                    debugPrefix();
                    std.debug.print("Run is now 0; checking next char\n", .{});
                    const next_char = input.line[i];
                    if (next_char == '?') {
                        debugPrefix();
                        std.debug.print("Next char is a '?', assuming that's a '.' and moving forward an additional space\n", .{});
                        i += 1;
                    }
                }
                debugPrint(input.line[i..], run, input.runs[r_i..]);
            ...
            '?' => {
                ...
                if (run != 0) {
                    ...
                    debugPrefix();
                    std.debug.print("Found '?'s while in a run; moving forward {} spaces\n", .{steps});
                    ...
                    if (run == 0 and i < input.line.len) {
                        ...
                        if (next_char == '#') {
                            debugPrefix();
                            std.debug.print("Next char is a '#'; invalid sequence\n", .{});
                            return 0;
                        }
                        if (next_char == '?') {
                            debugPrefix();
                ...
                if (last_run_done) {
                    debugPrefix();
                    std.debug.print("Found '?'s while no runs are available; assuming '.'s for {} spaces.\n", .{run_len});
                ...
                debugPrefix();
                std.debug.print("Branch point!\n", .{});

                debugPrefix();
                std.debug.print("What happens if we pretend the next '?' is a '.'?\n", .{});
                prefix_depth += 1;
                ...
                prefix_depth -= 1;
                debugPrefix();
                std.debug.print("Finalizing {s} (", .{input.line});
                std.debug.print("{}", .{input.current_run});
                for (input.runs) |r| {
                    std.debug.print(", {}", .{r});
                }
                std.debug.print(") as having {} arrangement(s)\n", .{arrangements});
                return arrangements;
            },
