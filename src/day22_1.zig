...
pub fn main() !void {
    ...
    // we are looking for every block whose upward neighbors have at least two downward neighbors
    ...
    var count: u64 = 0;
    var itr = m.iterator();
    blockloop: while (itr.next()) |e| {
        const ups = e.value_ptr.ups.items;
        for (ups) |u_idx| {
            const u = m.get(u_idx).?;
            std.debug.assert(u.downs.items.len >= 1);
            if (u.downs.items.len == 1) {
                continue :blockloop;
            }
        }
        count += 1;
    ...
    try stdout.print("Removable blocks: {}\n", .{count});
}
...
fn Grid(
    ...
    return struct {
        ...
        pub fn insertBlockFromCoordPair(self: *Self, a: [3]u16, b: [3]u16) !void {
            ...
            var idx: usize = 0;
            while (idx < self.blocks.items.len and self.blocks.items[idx].z2() <= block.z2()) : (idx += 1) {}
        ...
        // pub fn populateNeighbors(self: Self, map: *AutoHashMap(usize, struct { ups: ArrayList(usize), downs: ArrayList(usize) })) void {
        //     _ = map; // autofix
        ...
        //                     if (block.*.coords[0] != x or
        //                         block.*.coords[1] != y or
        //                         block.*.coords[2] != z) continue;

        //                     var ups = ArrayList(usize).init(self.allocator);
        //                     var downs = ArrayList(usize).init(self.allocator);
        ...
        //                             if (block.coords[2] > 0 and self.spaces[b_x][b_y][block.coords[2] - 1]) |idx| {
        //                                 downs.contains(idx) else {
        //                                     downs.append(idx);
        //                                 }
        //                                 downs.append(idx);
        //                             }
