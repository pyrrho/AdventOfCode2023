...
test "find rank" {
    ...
    try testing.expect(Hand.findRank("KTJJT") == Rank.four_of_a_kind);
    try testing.expect(Hand.findRank("QQQJA") == Rank.four_of_a_kind);
    try testing.expect(Hand.findRank("QQAJA") == Rank.full_house);
    try testing.expect(Hand.findRank("2J954") == Rank.one_pair);
    try testing.expect(Hand.findRank("J2954") == Rank.one_pair);
    try testing.expect(Hand.findRank("JJJJJ") == Rank.five_of_a_kind);
    try testing.expect(Hand.findRank("644J6") == Rank.full_house);
}
...
test "comparing hands" {
    ...
    try testing.expect(Hand.fromCards("QJJQ2", 0).lt(Hand.fromCards("QQQQ2", 0)) == true);
    try testing.expect(Hand.fromCards("JKKK2", 0).lt(Hand.fromCards("QQQQ2", 0)) == true);
}
